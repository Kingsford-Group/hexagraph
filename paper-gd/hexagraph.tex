...
adjacencies in order to reduce the number of explicit edges that must be drawn.
...
of explicit edges drawn and to maximize other aesthetic qualities. The approach is based on
the combination of several heuristics, the most central of which is the
...
relatively large number of adjacencies. Further, common motifs such as cliques,
...
bipartite matching) to refine a mapping $f$ from vertices to tiles. A
randomized approach that is used to avoid becoming trapped in local optima is
presented and tested as well. We also test an approach to edge drawing and
...
produce drawings that are inorganic or unintuitive to read. A similar iterative,
...
under several metrics such as small area, limited node-edge crossings, and a
...
adjacent tile of another node that is not an neighbor of $u$. In other words, a
...
can be changed to include benefit $ri(u,v)$ for each satisfied edge. For the experiments reported here, we take $r=1000$
and $d(c,d) = 500 / h_{cd}$ where $h_{cd}$ is a Euclidean distance between the
centers of tiles $c$ and $d$ divided by the hexagon radius.
...
matching. In this case, all the edge weights are $1$. Because the tiling field
...
of the resulting drawing, and the next island is considered. This process
continues until every island is in its locally optimal location.
...
powerlaw degree distribution (computed via~\cite{networkx}). This graph is
...
\textbf{Area} gives the of the drawing in terms of number of tiles.
...
Table~\ref{tbl:qual}. The running times are fairly practical: most graphs of
...
entire layout to be computed on a 3.4~GHz iMac in Python. Numbers are averaged
over $10$ randomized runs.}\label{tbl:time}
%
\centering
...
recomputing routes after each matching iteration.
...
improvement phase. These algorithms produce drawings are often produce very
